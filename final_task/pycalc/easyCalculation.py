...
class Calculator():
    ...
    def __calculation(self,expr):    
        ...
        while place!=None:
            #регульрное выражение с конца строки есть вроде
                ...
                rezult= '{:.15f}'.format( operators.operators[place[0]](float(findBefore[0][::-1]),float(findAfter[0])))
    ...
    def __degree (self,expr):
        ...
        while place!=-1:
            ...
            rezult= '{:.15f}'.format( operators.operators["^"](float(findBefore[0][::-1]),float(findAfter[0])))
