...
Functions:
    ...
    parse: splits the expression into numbers, CONSTANTS, FUNCTIONS, operations;
    check_expression: checks the validity of the entered expression;
    transform_to_polish_notation: converts an expression according to the rules of reverse polish notation;
    calculate: calculates written in reverse polish notation.

Attributes:
    OPERATORS: dictionary of operations and their priorities
    FUNCTIONS: dictionary of mathematical functions
    BUILT_IN_FUNCTIONS: dictionary of numeric functions
    CONSTANTS: dictionary math constants
    NEGATIVE_FLOAT_TYPE1: possible representations of a negative float number like -.1
    NEGATIVE_FLOAT_TYPE2: possible representations of a negative float number like -0.1
    NEGATIVE_INTEGER: possible representations of a negative int number like -1
    FLOAT_TYPE1: possible representations of a float number like 0.1
    FLOAT_TYPE2: possible representations of a float number like .1
    INTEGER: possible representations of a int number like -1
...
def py_calculator(math_expression: str):
    """
    ...
    def parse(expression: str) -> list:
        """Split math expression. Consists of 4 parsing steps.

        :param expression: String of math expression
        :return: Parsed string by item in list

        """
        def parse_step1(expr: str) -> list:
            """Creates a list with numbers, functions, operators and constants from the math expression.
        ...
        def parse_step2(parse_list: list) -> list:
            ...
            try:
                for index, element in enumerate(parse_list):
                    ...
                    elif(element in FUNCTIONS or element in BUILT_IN_FUNCTIONS) and parse_list[index + 1] != '(':
    ...
    def check_expression(parse_expression: list):
        ...
        """
        def check_operators(parse_list: list):
        ...
        def check_function_and_constants(parse_list: list):
            ...
            """
            copy_check_expression = parse_list.copy()
        ...
        def check_brackets(parse_list: list):
            """Check count of '(',')'.
    ...
    def transform_to_polish_notation(parse_expression: list) -> str:
        ...
        for token in parse_expression:
            if token == ''.join(INTEGER.findall(token)) \
                ...
                stack.append(token)
            elif token in FUNCTIONS or token in BUILT_IN_FUNCTIONS:
                stack.append(token)
                ...
                if not stack:
                    ...
                    stack.append(token)
                elif stack[-1] in FUNCTIONS or stack[-1] in BUILT_IN_FUNCTIONS:
                    reverse_polish_notation += stack.pop() + separator
                    if not stack:
                        stack.append(token)
            ...
            if token in OPERATORS:
                ...
                if stack[-2] == ',':
                    ...
                    if stack[-1] == ',':
                        ...
                        stack.append(BUILT_IN_FUNCTIONS[token](op1, int(op2)))
                else:
                    op = stack.pop()
                    stack.append(BUILT_IN_FUNCTIONS[token](op))
...
def main():
    try:
        ...
        res = py_calculator(args.EXPRESSION)
        print(res)
